...
RETRYABLE_NETWORK_EXCEPTIONS = (httpx.TransportError,)
...
class GDMClient:
    ...
    def generate_response(self, messages: list, functions: list = None, **model_kwargs):
        ...
        # Call the Gemini model with retry logic for transient errors
        ...
        for attempt in range(NUM_RETRIES + 1):
            try:
                ...
                # Check for network-level transport errors (DNS, connection, timeout)
                is_network_error = isinstance(e, RETRYABLE_NETWORK_EXCEPTIONS)
                ...
                is_retryable = is_network_error or is_http_retryable

                if is_retryable and attempt < NUM_RETRIES:
                    delay = min(RETRY_BASE_DELAY * (2 ** attempt) + random.uniform(0, 1), RETRY_MAX_DELAY)
                    error_type = type(e).__name__
                    logger.warning(f"Retryable {error_type} (attempt {attempt + 1}/{NUM_RETRIES}), retrying in {delay:.1f}s: {e}")
                    time.sleep(delay)
                else:
                    raise
