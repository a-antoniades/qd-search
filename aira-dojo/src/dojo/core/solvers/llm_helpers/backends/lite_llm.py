...
class LiteLLMClient:
    ...
    def __init__(self, client_cfg):
        ...
        # Only add a prefix if the model_id doesn't already contain a provider prefix (e.g. "gemini/...")
    ...
    def _query_client(
        ...
        try:
            ...
            if "function calling" in str(e).lower() or "functions" in str(e).lower() or "function_call" in str(e).lower():
                logger.warning(
    ...
    def query(
        ...
        **model_kwargs,
        ...
        # litellm extracts system messages into system_instruction, leaving contents empty.
        # Mirror gdm.py behavior by adding an empty user message.
        if "gemini" in self.model.lower() and all(m.get("role") == "system" for m in messages):
            messages.append({"role": "user", self.client_content_key: " "})
