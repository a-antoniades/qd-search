...
def _main(cfg: RunConfig):
    ...
    if solution is None:
        ...
        elif hasattr(best_node, "metric") and best_node.metric is not None and best_node.metric.value is not None:
            # Fallback: use metric.value (which is the test_score when use_test_score=True)
            log.info("Using metric value as final score...")
            fitness = best_node.metric.value
            log.info(f"Final fitness: {fitness}")
        else:
            raise ValueError("No valid score found in best_node.metric")
