...
from dojo.core.solvers.selection import get_selector, NodeSelector
...
class Evolutionary(Solver):
    def __init__(self, cfg: EvolutionarySolverConfig, task_info):
        ...
        # Initialize node selector (fitness-based by default, can be RLM-based)
        selector_cfg = getattr(cfg, "selector", None)
        self.node_selector: NodeSelector = get_selector(
            cfg=selector_cfg,
    ...
    def save_checkpoint(self):
        ...
        # Save RLM selection history if using RLM selector
        from dojo.core.solvers.selection.rlm_selector import RLMNodeSelector
        if isinstance(self.node_selector, RLMNodeSelector):
            history_path = Path(self.cfg.checkpoint_path) / "selection_history.jsonl"
            self.node_selector.save_history(history_path)

        # Save MAP-Elites archive state if using MAP-Elites selector
        from dojo.core.solvers.selection.mapelites_selector import MAPElitesNodeSelector
        if isinstance(self.node_selector, MAPElitesNodeSelector):
            archive_state = self.node_selector.get_archive_state()
            archive_path = Path(self.cfg.checkpoint_path) / "mapelites_archive.json"
            with open(archive_path, "w") as f:
                json.dump(archive_state, f, indent=2)
            self.logger.info(f"MAP-Elites archive saved to {archive_path}")
    ...
    def load_checkpoint(self):
        ...
        # Load RLM selection history if using RLM selector
        from dojo.core.solvers.selection.rlm_selector import RLMNodeSelector
        if isinstance(self.node_selector, RLMNodeSelector):
            history_path = Path(self.cfg.checkpoint_path) / "selection_history.jsonl"
            self.node_selector.load_history(history_path)
    ...
    def _draft(self, selection_context: Optional[str] = None) -> Node:
        """
        ...
        Args:
            selection_context: Optional context from RLM selector reasoning
    ...
    def _improve(self, parent_node: Node, selection_context: Optional[str] = None) -> Node:
        """
    ...
    def _crossover(self, parent_node1: Node, parent_node2: Node, selection_context: Optional[str] = None) -> Node:
        """
        Combine two solutions using the crossover LLM operator.

        Args:
            parent_node1: First parent node
            parent_node2: Second parent node
            selection_context: Optional context from RLM selector reasoning

        Returns:
            Node: A new node containing the combined solution
        """
        plan, code, metrics = execute_op_plan_code(
            self.crossover_fn,
            self.task_desc,
    ...
    def parse_eval_result(self, node: Node, eval_result: Dict[str, Any]):
        ...
        # When using test score, the grader is ground truth — don't let a failed
        # analyze parse (which defaults is_bug=True) override a valid graded score.
        if self.cfg.use_test_score and response["metric"] is not None and valid_solution:
            node.is_buggy = not node.exit_code == 0
        else:
            node.is_buggy = (
    ...
    def search(self, task, state) -> Optional[str]:
        ...
        for generation_id in range(self.state.current_generation, self.cfg.num_generations):
            ...
            for counter_id in range(self.cfg.individuals_per_generation):
                # Selection context for operators (RLM selector reasoning)
                current_selection_reasoning: Optional[str] = None
                ...
                if generation_id == 0:  # initially, uniformly populate the islands
                    ...
                    # Use node selector for intelligent selection
                    crossover_prob = (
                        0 if generation_id < self.cfg.num_generations_till_crossover
                        else self.cfg.crossover_prob
                    )
                    # Calculate step info for step-aware selection
                    total_steps = self.cfg.num_generations * self.cfg.individuals_per_generation
                    current_step = generation_id * self.cfg.individuals_per_generation + counter_id
                    selection_context = {
                        "temperature": temperature,
                        "crossover_prob": crossover_prob,
                        "num_samples": self.cfg.few_shot,
                        "islands": solution_database._islands,
                        # Step-awareness for RLM selector
                        "current_step": current_step,
                        "step_limit": total_steps,
                        "steps_remaining": total_steps - current_step,
                    }
                    selection_result = self.node_selector.select(
                        journal=self.journal,
                        context=selection_context,
                    )

                    # Extract nodes and metadata from selection result
                    in_context_nodes = selection_result.nodes
                    island_id = selection_result.island_id if selection_result.island_id is not None else 0
                    operator = selection_result.operator

                    # Store selection reasoning to pass to operators (useful for RLM selector)
                    if selection_result.reasoning:
                        current_selection_reasoning = selection_result.reasoning
                        self.logger.debug(
                            f"Selection reasoning: {selection_result.reasoning}", LogEvent.SOLVER
                        )
                ...
                child_node = create_node_fn(*in_context_nodes, selection_context=current_selection_reasoning)
                state, eval_result = task.step_task(state, extract_code(child_node.code))
                self.parse_eval_result(child_node, eval_result)

                # Record outcome for RLM selection history (only for gen > 0)
                if generation_id > 0:
                    from dojo.core.solvers.selection.rlm_selector import RLMNodeSelector
                    if isinstance(self.node_selector, RLMNodeSelector):
                        child_metric = child_node.metric.value if child_node.metric else None
                        self.node_selector.record_outcome(
    ...
    def log_journal(self):
        ...
        node_data = self.journal.get_node_data(self.state.current_step) | {"current_best_node": best_node_step}

        # Append QD metrics if using MAP-Elites selector
        from dojo.core.solvers.selection.mapelites_selector import MAPElitesNodeSelector
        if isinstance(self.node_selector, MAPElitesNodeSelector):
            archive_state = self.node_selector.get_archive_state()
            if archive_state.get("initialized"):
                node_data["qd_coverage"] = archive_state["coverage"]
                node_data["qd_score"] = archive_state["qd_score"]
                node_data["qd_archive_size"] = archive_state["archive_size"]
