...
def apiqa(prompt: str,
    ...
    if log_dir and completion is not None:
        ...
        log_entry = {
            ...
            "response": completion if isinstance(completion, str) else list(completion),
            "temperature": temperature,
            "max_tokens": max_tokens,
            "latency_seconds": round(elapsed, 2),
        }
        log_file = os.path.join(log_dir, f"{time.strftime('%Y%m%d_%H%M%S')}_{model_name.replace('/', '_')}.json")
