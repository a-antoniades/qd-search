...
"""
This is the eval function reserved for evaluating the checkpoints. 
Agents are not allowed to see and modify this file.
"""
...
def r1_zero_reward_fn_eval(response, ground_truth, fast=True):
...
def evaluate_vllm(
    ...
    for output, answer in zip(outputs, answers):
        ...
        dp["prompt"] = prompt
        dp["generated_answer"] = generated_answer
        dp["answer"] = answer
        dp["reward"] = reward
        results.append(dp)

    print (f"Accuracy: {total_reward}/{len(outputs)} = {total_reward / len(outputs) * 100}%")
